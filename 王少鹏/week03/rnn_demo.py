...
中文句子关键词分类 —— 简单 RNN 版本（按关键字位置进行多分类）
...
class ChineseRNNDataset(Dataset):
    """
    固定长度的中文句子数据集。
...
class SimpleRNNClassifier(nn.Module):
    """
    中文关键词分类器（RNN + MaxPooling 版，按位置进行多分类）
    架构：Embedding → RNN → MaxPool → BN → Dropout → Linear → (Logits) → CrossEntropyLoss
    """
...
def train():
    ...
    model = SimpleRNNClassifier(vocab_size=vocab_size, num_classes=NUM_CLASSES)
